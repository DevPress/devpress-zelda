 ...
 */
function zelda_wp_title( $title, $sep ) {
	if ( is_feed() ) {
		return $title;
	}
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		$title .= " $sep " . sprintf( __( 'Page %s', 'zelda' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'zelda_wp_title', 10, 2 );
 ...
 */
function zelda_get_menu_name( $location ) {
